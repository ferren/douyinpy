...
def check_signature(token, signature, timestamp, nonce):
    """Check Douyin callback signature, raises InvalidSignatureException
    if check failed.

    :param token: Douyin callback token
    :param signature: Douyin callback signature sent by Douyin server
    :param timestamp: Douyin callback timestamp sent by Douyin server
    :param nonce: Douyin callback nonce sent by Douyin sever
    """
    signer = DouyinSigner()
    signer.add_data(token, timestamp, nonce)
    if signer.signature != signature:
        from douyinpy.exceptions import InvalidSignatureException
