...
"""
    ...
    This module provides client library for Douyin Open Platform
...
from douyinpy.client import DouyinComponentClient
from douyinpy.constants import DouyinErrorCode
from douyinpy.crypto import DouyinCrypto
from douyinpy.exceptions import (
    APILimitedException,
    DouyinClientException,
    DouyinComponentOAuthException,
    DouyinOAuthException,
)
from douyinpy.messages import COMPONENT_MESSAGE_TYPES, ComponentUnknownMessage
from douyinpy.session.memorystorage import MemoryStorage
from douyinpy.utils import to_text

logger = logging.getLogger(__name__)


class BaseDouyinComponent:
    ...
    def __init__(
        ...
        self.crypto = DouyinCrypto(component_token, encoding_aes_key, component_appid)
...
class DouyinComponent(BaseDouyinComponent):
    ...
    def get_client_by_appid(self, authorizer_appid):
        ...
        return DouyinComponentClient(authorizer_appid, self, session=self.session)

    def parse_message(self, msg, msg_signature, timestamp, nonce):
        """
        处理 douyin server 推送消息
...
class ComponentOAuth:
    ...
    def __init__(self, component, app_id):
        """
        :param component: DouyinComponent
    ...
    def get_authorize_url(self, redirect_uri, scope="snsapi_base", state=""):
        ...
        url_list.append("#douyin_redirect")
    ...
    def _handle_result(self, res, method=None, url=None, **kwargs):
        ...
        if "errcode" in result and result["errcode"] != 0:
            ...
            if self.component.auto_retry and errcode in (
                ...
                raise DouyinComponentOAuthException(errcode, errmsg, client=self, request=res.request, response=res)
        return result
